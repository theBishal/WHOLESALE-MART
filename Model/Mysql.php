...
class Mysql
{
    ...
    public function check_token_reset_password($selector, $token, $new_password){
        $current_date = date("U");
        $sql = "SELECT * FROM pwd_reset WHERE selector='$selector' AND expires>='$current_date' ORDER BY id DESC";
        global $conn;
        if($result = mysqli_query($conn, $sql)){
            $rows = array();
            while($r = mysqli_fetch_assoc($result)) {
            ...
            if(password_verify($tokenBin, $rows[0]['token'])){
                $hash_password = password_hash($new_password, PASSWORD_DEFAULT);
                $sql2 = "UPDATE users SET password = '$new_hash_password' WHERE email='$rows[0]['email']'";
